	...
	async.waterfall([
			function(callback) {
				//console.log('1st Step');
				 logger.info('1st Step Historical ARPI');
			...
			function(organization, reportCollection, isAdmin, callback) {
				...
				logger.info('2nd Step Historical ARPI');
			...
			function(returnReportCollection, callback) {
				...
				logger.info('3rd Step Historical ARPI');
				var gridParams = {
		...
		function(err, prepareGridAndChartData, preparefilters) {
			if(err){
				...
				res.render('historical_linechart', {
					"charttype": "line",
				...
				httpClient.request(reportURL,
					function(err, httpres, body) {
						if (err) {
							...
							var dateMax = dateFormat(now, "yyyy-mm-dd");
							var dateMin = dateFormat(now.setDate(now.getDate() - 30), "yyyy-mm-dd"); // one year gap
							var collection = {
								dateMin: dateMin,
								dateMax: dateMax
							};
...
router.get('/gridviewfilterchange', function(req, res) {
	logger.info("INFO - historical_linechart route -- IN gridviewfilterchange");
...
router.get('/expfile', function(req, res) {
	...
	var gameTitle = req.query.gameTitle;
	var organization = req.query.org;
	var filename = "historical-arpi-" + shortid.generate();
	...
	var MysqlurlGrid = "";
	MysqlurlGrid = '/api/historicalapiGridView?mindate=' + encodeURIComponent(gridParams.dateMin) + '&maxdate=' + encodeURIComponent(gridParams.dateMax);
...
var getGameTitlesByOrg = function(organization, callback) {
	...
	httpClient.request(requrl,
		function(err, httpres, body) {
			if (err) {
				//console.log('historical_linechart_route -- Error in httprequest to get titles by organization--', err);
				 logger.error(new Error('historical_linechart_route -- Error in httprequest to get titles by organization--' + err));
				response.error = err;
				callback(response);
			} else if (httpres.statusCode != config.codes.OK) {
				//console.log('historical_linechart_route -- Error in httprequest to get titles by organization--', httpres);
				logger.error(new Error('historical_linechart_route -- Error in httprequest to get titles by organization--' + httpres));
				response.error = body;
				callback(response);
			} else if (body) {
...
var filterDataByDate = function(reportCollection, callback) {
	...
	httpClient.request(Mysqlurlfordropdownbinding,
		function(err, httpres, body) {
			if (err) {
				//console.log('historical_linechart_routes -- Error in http request - Mysqlurl', err);
				 logger.error(new Error('historical_linechart_routes -- Error in http request - Mysqlurl' + err));
				response.error = 'historical_linechart_routes -- Error in http request - Mysqlurl';
				callback(response);
...
var getGridAndChartData = function(gridParams, callback) {
		...
		async.parallel([function(callback) {
			var MysqlurlGrid = "";
			MysqlurlGrid = '/api/historicalapiGridView?mindate=' + encodeURIComponent(dateMin) + '&maxdate=' + encodeURIComponent(dateMax);
			MysqlurlGrid += '&platform=' + (JSON.stringify(gridParams.gridPlatform));
			MysqlurlGrid += '&sources=' + (JSON.stringify(gridParams.gridSource));
			MysqlurlGrid += '&country="' + encodeURIComponent(gridParams.countryvalue) + '"';
			MysqlurlGrid += '&organiclift=' + encodeURIComponent(true);
			...
			logger.info("INFO - historical_linechart route -- calling historicalapiGridView with url");
			logger.info(MysqlurlGrid);

			httpClient.request(MysqlurlGrid,
				function(err, httpres, body) {
					if (err) {
						//console.log('historical_linechart -- Error in http request for MysqlurlGrid', err);
						logger.error(new Error('historical_linechart -- Error in http request for MysqlurlGrid' + err));
						return callback(err);
					} else if (httpres.statusCode != config.codes.OK) {
						logger.error(new Error('historical_linechart -- Error in http request for MysqlurlGrid'));
			...
			var chartUrl = "";
			chartUrl = '/api/historicalapiChartView?mindate=' + encodeURIComponent(dateMin) + '&maxdate=' + encodeURIComponent(dateMax);
			...
			httpClient.request(chartUrl,
				function(err, httpres, body) {
					if (err) {
						// console.log('historical_linechart -- Error in http request for chartUrl', err);
						logger.error(new Error('historical_linechart -- Error in http request for chartUrl' + err));
						return callback(err);
					}
					if (body) {
