	    	...
	    	if(role == "SuperAdmin"){
	    		...
	    		getUserData(email, reporttitlename, function(user_data){
	    			...
	    			getReportCollection(titleQuery, user_data, function(reports){
	    				...
	    				//console.log('pankaj--',reports.reportCollection);
...
router.get('/expfile',function(req, res){
	...
	var gameTitle = req.query.gameTitle;
	var organization = req.query.org;
	//var filename = "ua-dashboard-"+shortid.generate();
	
	mindate = dateFormat(mindate.substring(0, 10).replace(/\-/g, '/'), "yyyy-mm-dd");
	maxdate = dateFormat(maxdate.substring(0, 10).replace(/\-/g, '/'), "yyyy-mm-dd");
	var datesForExportName = dateFormat(mindate.substring(0, 10).replace(/\-/g, '/'), "mm-dd-yy") +'_'+dateFormat(maxdate.substring(0, 10).replace(/\-/g, '/'), "mm-dd-yy");
...
res.render('partials/dashboardGrid', {
...
res.render('partials/dashboardGridBody', {
...
var getAdminData = function(callback){
	logger.info('INFO - ua_dashboard_route -- In getAdminData');
	var options = {
	    hostname: 'localhost',
	    path: '/',
	    port: 3000,
		...
		httpClient.request('/admindata',  
			        function (err, httpres, body) {
						if(err){
							...
							logger.error(new Error('ERROR - In httpreqest for admindata api'+ httpres.statusCode));
				    		...
				    		logger.info('INFO - Admin User data retrived successfully');
				    		callback(userdata);
						}
			          });
}
...
var filterDataByDate = function(reportCollection, callback){
	...
	httpClient.request(Mysqlurlfordropdownbinding, 
		    	function (err, httpres, body) {
		    		if(err){
		    			//console.log('ua_dashboard_routes -- Error in http request - Mysqlurl',err);
		    			logger.error(new Error('ua_dashboard_routes -- Error in http request - Mysqlurl' + err));
		    			response.error = 'ua_dashboard_routes -- Error in http request - Mysqlurl';
		    			callback(response);
...
var getGridAndChartData = function(gridParams, callback){
  ...
  //     		var horizon_value = gridParams.horizon_value;
		...
		// 			function (err, httpres, body) {
		// 				if(err){
		// 					//console.log('ua_dashboard -- Error in http request for MysqlurlGrid',err);
		// 					logger.error(new Error('ua_dashboard -- Error in http request for MysqlurlGrid' + err));
		// 					return callback(err);
		// 				} else if(httpres.statusCode != config.codes.OK){
		// 					logger.error(new Error('ua_dashboard -- Error in' + MysqlurlGrid));
		// 					return callback('ua_dashboard -- Error in' + MysqlurlGrid);
		// 				} else if(body){
		// 					var apiMysqlurlGridResponse = JSON.parse(body);
  ...
  //             					"horizon_value": horizon_value,
		...
		function(callback){
			...
			httpClient.request(chartUrl,
					function (err, httpres, body) {
						if(err){
							//console.log('ua_dashboard -- Error in http request for chartUrl',err);
							logger.error(new Error('ua_dashboard -- Error in http request for chartUrl' + err));
							return callback(err);
						} else if(httpres.statusCode != config.codes.OK) {
							//console.log('ua_dashboard -- Error in' + chartUrl);
							logger.error(new Error('ua_dashboard -- Error in' + chartUrl));
							return callback('ua_dashboard -- Error in' + chartUrl);
						} else if(body){
