		...
		function(err, prepareGridAndChartData, preparefilters) {
			if(err){
				...
				var t2 = parseInt(Object.keys(pair.data).length/2);
					...
					//console.log("---0", (_.where(pair.data, {"isSignificant": 0}).length));
					series.push(pair);
				}
...
router.get('/gridviewfilterchange', function(req, res) {
	logger.info("INFO - historical_arpi route -- IN gridviewfilterchange");
...
router.get('/expfile', function(req, res) {
	...
	var filename = "historical-source-arpi_" + req.query.gameTitle.replace(/\ /g, '_') + "_"+ datesForExportName;
	var gridPlatform = [];
	var gridSource = [];
		...
		function(err, httpres, body) {
			if (err) {
				//console.log('historical_arpi_route -- Error in httprequest to get titles by organization--', err);
				 logger.error(new Error('historical_arpi_route -- Error in httprequest to get titles by organization--' + err));
				response.error = err;
				callback(response);
			} else if (httpres.statusCode != config.codes.OK) {
				//console.log('historical_arpi_route -- Error in httprequest to get titles by organization--', httpres);
				logger.error(new Error('historical_arpi_route -- Error in httprequest to get titles by organization--' + httpres));
				response.error = body;
				callback(response);
			} else if (body) {
		...
		function(err, httpres, body) {
			if (err) {
				//console.log('historical_arpi_routes -- Error in http request - Mysqlurl', err);
				 logger.error(new Error('historical_arpi_routes -- Error in http request - Mysqlurl' + err));
				response.error = 'historical_arpi_routes -- Error in http request - Mysqlurl';
				callback(response);
			...
			httpClient.request(chartUrl,
				function(err, httpres, body) {
					if (err) {
						...
						logger.error(new Error('historical_arpi -- Error in http request for chartUrl' + err));
						return callback(err);
					}
					if (body) {
