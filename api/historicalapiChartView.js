...
function GetCountryClause4Chart(country) {
    ...
    if (country.indexOf("All") > -1) {      
        countryqueryclause.orderbyclause = "ORDER BY  source, ProjectionDate";
    } else {        
        countryqueryclause.country_part = format("AND country in  ({0}) ", country);
    }

    countryqueryclause.groupbyclause = "GROUP BY  source, ProjectionDate ";
...
function GetRawSqlQuery4DefaultChart() {
    ...
    var selectquery =   "SELECT "+
                        ...
                        "UI_data "+
                        "WHERE PlayerCount > 0 " +
                        "GROUP BY  source, ProjectionDate " +
