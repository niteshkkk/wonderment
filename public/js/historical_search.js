...
function OnPagingNumberclick() {
  ...
  document.querySelector(".txtpagenumber").addEventListener("keypress", function(evt) {
   ...
   // $('.txtpagenumber').on('blur', function()
   //  {
...
function exportcsv() {
  $(".export-csv").click(function(e) {
    ...
    $.ajax({
      url: "/historical_source_arpi/expfile",
      data: reportDatanew,
      method: "get",
      type: 'get'
...
function datechange() {
  ...
  if ($('#dtp_input2').val() && $('#dtp_input3').val()) {
    if (cohortStartDate > cohortEndDate) {
      ...
      $.ajax({
        url: "/historical_source_arpi/cohortchange",
        method: "get",
        type: 'get',
        data: reportDatanew,
...
function reverdata() {
  if (!$("#revert").attr("disabled") && $("#revert").attr("class") == "revert") {
    ...
    var organization = $("#orgna").text().trim();
    $.ajax({
      url: "/historical_source_arpi/title?title=" + title + "&reporttitlename=" + reporttitlename + "&organization=" + organization,
      method: "get",
      type: 'get',
    }).success(function(data) {
      // console.log('data',data);
...
function hsitoricalgrid(reportDatanew) {
  similartoreportselected();
  $.ajax({
    url: "/historical_source_arpi/gridviewfilterchange",
    method: "get",
    type: 'get',
    data: reportDatanew,
    contentType: "application/json; charset=utf-8",
...
$.ajax({
        url: "/historical_source_arpi/getGriddata",
        method: "get",
        type: 'get',
        data: reportDatanew,
        contentType: "application/json; charset=utf-8",
