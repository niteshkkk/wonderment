...
$(document).ready(function() {
  ...
  onReportClick('historical_platform_arpi');
...
//on selection of data shown per page and typing page number click
...
function exportcsv() {
  $(".export-csv").click(function(e) {
    ...
    $.ajax({
      url: "/historical_platform_arpi/expfile",
      data: reportDatanew,
      method: "get",
      type: 'get'
      ...
      //window.open('/download/'+data+'.csv','_blank');
...
function showPageIndex(ctrl, currentIndex) {
  ...
  var filterData = historicalData.slice(startDataRange, (parseInt(startDataRange) + parseInt(recordsToShow)));
...
function pagingrid() {
  //historicalData = JSON.parse(unescape($('input[name=gridData]').val()));
  totolNumberofRecords = historicalData.length;

  if ($('form.table-pagination select option:selected').length > 0) {
    recordsToShow = $('form.table-pagination select').find(":selected").text();
  }
  var filterData = [];
  //filterData =_.where(historicalData, query );

  filterData = historicalData.slice(0, recordsToShow);
  ...
  $('ul#platformmenu input[type=checkbox]').unbind().click(function() {
    ...
    if (this.id === 'all') {
      ...
      if (checkboxcount > 0 && checkboxcount == $('ul#platformmenu input[type=checkbox]').length - 1) {
        checkedValues = "All";
        $('ul#platformmenu #all').prop('checked', true);
...
function datechange() {
  ...
  if ($('#dtp_input2').val() && $('#dtp_input3').val()) {
    if (cohortStartDate > cohortEndDate) {
      ...
      $.ajax({
        url: "/historical_platform_arpi/cohortchange",
        method: "get",
        type: 'get',
        data: reportDatanew,
...
function reverdata() {
  if (!$("#revert").attr("disabled") && $("#revert").attr("class") == "revert") {
    ...
    var organization = $("#orgna").text().trim();
    $.ajax({
      url: "/historical_platform_arpi/title?title=" + title + "&reporttitlename=" + reporttitlename + "&organization=" + organization,
      method: "get",
      type: 'get',
    }).success(function(data) {
      // console.log('data',data);
...
function hsitoricalgrid(reportDatanew) {
  similartoreportselected();
  $.ajax({
    url: "/historical_platform_arpi/gridviewfilterchange",
    method: "get",
    type: 'get',
    data: reportDatanew,
    contentType: "application/json; charset=utf-8",
...
$.ajax({
        url: "/historical_platform_arpi/getGriddata",
        method: "get",
        type: 'get',
        data: reportDatanew,
        contentType: "application/json; charset=utf-8",
